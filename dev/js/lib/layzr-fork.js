...
export default (options = {}) => {
  ...
  function setSource (node) {
    ...
    ;[settings.normal, settings.retina, settings.srcset].forEach(attr =>
  ...
  function update () {
    nodes = Array.prototype.slice
      .call(document.querySelectorAll(`[${settings.normal}]`))
      // This has been added so that images hidden via 'display:none' aren't loaded
      .filter(el => {
        return !!(
          el.offsetWidth ||
          el.offsetHeight ||
